...
Ext.onReady(function() {
	var API = {
			...
			copyModule : rootPath + "/module/copyModule",
	...
	var moduleTreeStore = Co.treeStore("moduleTreeStore", API.getModuleTreeLeft, {
		...
		listeners : {
			load : function(){
				var ss = moduleTreePanel.getSelectionModel();
				//默认选中上一次选中的组织
				var record = this.getNodeById(selectedModuleNo);
				if(record){
					Co.load(moduleGridStore, {"model.parentModuleNo" : selectedModuleNo});
					return;
				}
				//默认选中第一个
	...
	function loadModuleTreeStore(){
		Co.load(moduleTreeStore,{"selectedModuleNo":selectedModuleNo});
	}
	
	//============================ Model =========================
	Co.defineModel("Module", ["id","moduleNo","parentModuleNo","moduleName","moduleIcon","moduleType","moduleProperty","moduleUrl","moduleModule","moduleOrder","moduleShow","moduleDesc","moduleSourceUrl"]);
	//============================ Store =========================
	//列表Store
...
//		maxLengthText: "模块名称最多不能超过64字符",
	...
	var moduleCopyForm = Co.form(null, [{
		xtype : "numberfield",
		id : "copyNum",
		name : "copyNum",
		fieldLabel : "复制数量",
	...
	var moduleCopyFormWindow = Co.formWindow("复制图标", moduleCopyForm, 450, 160, "fit", {
		okHandler : function(){
			...
			exec_copyModule(Ext.getCmp("modelId").getValue(),copyNum);
		}
	});
	 ...
	 * 复制。默认复制1条
	 */
	function copyModuleMulti(){
		var record = Co.getGridSelectSingleRecord(moduleGrid);
		if(record){
			Co.resetForm(moduleCopyForm, true);
			Ext.getCmp("modelId").setValue(record.data.id);
			moduleCopyFormWindow.show();
		}
	}
	...
	function copyModule(){
		var record = Co.getGridSelectSingleRecord(moduleGrid);
		if(record){
			exec_copyModule(record.data.id,1);
		}
	}
	 ...
	 * 执行复制
	 */
	function exec_copyModule(moduleId,copyNum){
		Co.showConfirm("确定要复制吗？", function(btn){
			if (btn == "ok") {
				Co.request(API.copyModule,{"modelId":moduleId,"copyNum":copyNum},function(result){
					if(result.success){
						Co.alert("复制成功！",function(){
							Co.reload(moduleGridStore);
							moduleCopyFormWindow.hide();
						})
					} else {
