	...
	var orgTreeStore = Co.treeStore("orgTreeStore", API.getOrgTreeLeft, {
		...
		listeners : {
			load : function(){
				...
				//默认选中上一次选中的组织
				var selected_orgNo = Co.isEmpty(selectedOrgNo) ? orgNo : selectedOrgNo;
				var record = this.getNodeById(selected_orgNo);
				if(record){
					if(Co.isEmpty(selectedOrgNo)){
						ss.select(record);
					} else {
						Co.load(orgGridStore, {"model.parentOrgNo" : selectedOrgNo});
					}
					return;
				}
				//默认选中第一个
				...
				//没有任何一个组织
				Co.load(orgGridStore, {"model.parentOrgNo" : null});
			}
		}
	});
